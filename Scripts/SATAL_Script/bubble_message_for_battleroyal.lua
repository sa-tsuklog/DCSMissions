do
    ...
    trigger.action.setUserFlag('63', false) --out of buffer zone sound
    local numPlayersInBubble = trigger.misc.getUserFlag('64')
    trigger.action.setUserFlag('65', false) --out of bubble sound
    local numPlayersOutOfBubble = trigger.misc.getUserFlag('66')
    ...
    for clientId, clientData in pairs(mist.DBs.humansById) do
        ...
        if unit ~= nil then
            ...
            if (mDistance > mBubbleSize) then
                ...
                playerNamesInBubble[#playerNamesInBubble+1] = unit:getPlayerName()
    ...
    trigger.action.outText("ASF [0:0] XXX(1st Half) CZ Radius: ".. nmBubbleSize.."nm TTS: "..bubbleShrinkCount, 1,true)

    if(bubbleActiveCount ~= 0)then
        if(#playerNamesInBubble + #playerNamesInBufferZone > 0)then
            local names = ""
            for id,name in pairs(playerNamesInBufferZone) do
                names = name .. ", " .. names
            end 
            for id, name in pairs(playerNamesInBubble) do
                names = name .. ", " .. names
            end
            trigger.action.outText("Entered CombatZone: " .. names, 1,false)
        end
    else
        if (#playerNamesInBubble > 0)then
            local names = ""
            for id,name in pairs(playerNamesInBubble) do
                names = name .. ", " .. names
            end
            trigger.action.outText("In CombatZone: " .. names, 1,false)
        ...
        if (#playerNamesOutOfBubble > 0)then
            ...
            trigger.action.outText("Out Of CombatZone: " .. names, 1,false)
        end

        if(#playerNamesInBubble > numPlayersInBubble)then
            trigger.action.setUserFlag('63', true)
        end
        if(#playerNamesOutOfBubble > numPlayersOutOfBubble)then
            trigger.action.setUserFlag('65',true)
        end
    end

    trigger.action.setUserFlag('64',#playerNamesInBubble)
    trigger.action.setUserFlag('66',#playerNamesOutOfBubble)
