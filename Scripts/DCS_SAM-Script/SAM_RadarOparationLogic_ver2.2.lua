-- DCSのSAMを強化するスクリプトです。
-- Maigo-2-1さんによる対応を含んでいます。
-- https://github.com/Maigo-2-1
...
function airDefence()
	...
	-- 撃破・消去されたSAMを除外
	function removeDeadFromSamSet(_SamSetList)
		
		local _j = #_SamSetList
		while _j > 0 do
			if _SamSetList[_j].Unit:isExist() then
			else
				table.remove(_SamSetList , _j)
			end
			_j = _j - 1
		end
	end
		...
		for _i, _grp in pairs(coalition.getGroups(_coalitionSideNum)) do
			if string.find(_grp:getName(), _KEYOFDESIGNATEDSAMGRP) then
				...
				for _index, _unitObj in pairs(_grp:getUnits()) do
					...
					local duplicate = false
					for _j, _existing in pairs(_objList[_coalitionSideNum].SamSetList) do
						if _unitObj:getID() == _existing['Unit']:getID() then
							duplicate = true
							break
						end
					end
			...
			for _, _unit in pairs(_grp:getUnits()) do
				if _unit:isExist() then
					if _unit:getRadar() then
						if string.find(_unit:getName(), _ALWAYSACTIVATED) == nil then
	...
	function isArmHot(_arm, _radar)
		-- ポジション取得
		if not _radar:isExist() and not _arm.object:isExist() then
			return false
		end
	...
	function canShootMissile(_coalitionSideNum, _targetList, _range, _launcher)
		...
		for _, _target in pairs(_targetList['AviationTargetList']) do
			if _target.object:isExist() then
